...
ので冪等性を持たせながら環境設定する為に使います。読者にとってフローを追うことは本質ではなく、良くないものを修正することが本質である。そのため、少々キモいですがroleは分けず、モノリシックに管理します。あるアプリがなんのパッケージマネージャを用いて入れるのかとか気にせず、セマンティクスを明確に分けることが出来たらいいよね。
