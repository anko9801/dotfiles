    ...
    initContent = lib.mkMerge [
      ...
      ''
        ...
        obsidian-daily-note() {
          ...
          local dn_folder=$(${pkgs.jq}/bin/jq -r '.folder' "$dn_config_path")
          local dn_path_format=$(${pkgs.jq}/bin/jq -r '.format' "$dn_config_path" | _convert-date-format)
...
EOF
        ...
        obsidian-unique-note() {
          ...
          if [[ -f "$un_config_path" ]]; then
            un_folder=$(${pkgs.jq}/bin/jq -r '.folder' "$un_config_path")
            un_prefix_format=$(${pkgs.jq}/bin/jq -r '.format' "$un_config_path" | _convert-date-format)
          else
